...
<div class="orderViewCaption oh">
    <div class='line'>
        <h4 class="cursive">{t}Заказ №{/t}{$order.order_num}</h4>    
    ...
    {foreach $type_object->getDocsName() as $key=>$doc}
       <a href="{$type_object->getDocUrl($key)}" class="btn" style='padding; 0' target="_blank">{$doc.title}</a>
    {/foreach}
    {/if}
    {if $order->canOnlinePay()}
        <a href="{$order->getOnlinePayUrl()}" class="btn" style='padding; 0'>{t}оплатить{/t}</a><br>
    {/if}
</div>
...
<a href="{$router->getUrl('shop-front-cartpage', ['Act'=>'repeatOrder', 'order_num' => $order.order_num])}" rel="nofollow"
        class="btn orderViewRepeatOrder" style='padding: 0 10px; margin-bottom: 0; margin-right: 10px'>{t}Повторить заказ{/t}</a>
{if $order->canChangePayment()}
    <a href="{$router->getUrl('shop-front-myorderview', ['Act'=>'changePayment', 'order_id' => $order.order_num])}" rel="nofollow" class="btn btn-secondary inDialog" style='padding: 0 10px'>{t}Изменить оплату{/t}</a>
{/if}
<table class="orderInfo order-table striped responsive-table">
    <tbody>
