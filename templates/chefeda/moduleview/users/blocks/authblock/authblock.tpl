...
<div class="authorized">
    ...
    <ul class="dropdown">
        <li class="userInfo">
            ...
            <span class="balance">{t}Баланс:{/t}&nbsp;{hook name="users-blocks-authblock:balance" title="{t}Блок авторизации:баланс{/t}"}
                {$current_user->getBalance(true, true)}{/hook}
            </span>
            {/if}            
        </li>
        <hr>
...
<div class="auth alignright">
    ...
    <a href="{$router->getUrl('users-front-auth', ['referer' => $referer])}" class="auth inDialog" title="{t}Войти или зарегистрироваться{/t}"></a>
</div>
{/if}