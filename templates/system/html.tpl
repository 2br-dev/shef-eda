...
{$js.params.before}<script {if $js.params.type}type="{$js.params.type}"{/if} src="{$js.file}"></script>{$js.params.after}
{/foreach}
{if $app->getJsCode()!=''}
...
<body {if $app->getBodyClass()!= ''}class="{$app->getBodyClass()}"{/if} {$app->getBodyAttrLine()}>
    ...
    {$js.params.before}<script {if $js.params.type}type="{$js.params.type}"{/if} src="{$js.file}" defer></script>{$js.params.after}
    {/foreach}    
</body>
</html>