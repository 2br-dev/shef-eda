<div class="supportTopics">
    ...
    <div class="writeBlock mwHalf {if $error}open{/if}">
        <a href="#" class="button color handler" onclick="$(this).parent().addClass('open'); return false;">{t}Написать сообщение{/t}</a>
        <div class="caption">
            <h2>{t}Написать сообщение в поддержку{/t}</h2>
    ...
    <table class="themeTable supportTable">
        ...
        <tbody>
            ...
            <tr data-id="{$item.id}">
                ...
                <td class="remove">
                    <a href="{$router->getUrl('support-front-support', ["Act" => "delTopic", "id" => $item.id])}" class="iconRemove" title="{t}Удалить переписку по этой теме{/t}"></a>
                </td>
            </tr>
            {/foreach}
        </tbody>
