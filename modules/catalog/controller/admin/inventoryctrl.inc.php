...
class InventoryCtrl extends \RS\Controller\Admin\Crud
{
    ...
    function actionEnableControl()
    {
        ...
        $helper = new CrudCollection($this, null, null, array(
            'topTitle' => t('Включить складской учет'),
            'bottomToolbar' => new Toolbar\Element(array(
                    'Items' => array(
                        'save' => new ToolbarButton\SaveForm($this->router->getAdminUrl('EnableControlBySteps', null, 'catalog-inventoryctrl'), t('Продолжить'), array('attr' => array('class' => 'crud-get btn-success'))),
                        new ToolbarButton\Cancel($this->router->getAdminUrl('edit', array('mod' => 'catalog'), 'modcontrol-control'))
                    ))
            ),
            'viewAs' => 'form'
        ));
        ...
        $formObject = new \RS\Orm\FormObject(new \RS\Orm\PropertyIterator(
            array(
                'file' => new Type\File(array(
                    ...
                    'allow_file_types' => array('csv'),
                )),
                'example_file' => new Type\Integer(array(
    ...
    function actionAddProductsFromCatalog()
    {
        ...
        $form_object = new FormObject(new PropertyIterator(array(
            ...
            'document_id' => new \Catalog\Model\OrmType\SelectDocument(array(
                'type' => $type,
                'minLength' => 1,
                'attr' => array(array('size' => 40, 'placeholder' => t('id документа')))
            ))
        )));
        ...
        if ($this->url->isPost()) {
            ...
            if ($form_object->checkData()) {
                ...
                $ids = $this->url->request('ids', TYPE_ARRAY, array(), false);
                ...
                $crud_add_url = $api->getControllerUrlByDocumentType($type, $do, array('document_type' => $type, 'id' => $document_id, 'exist' => $exist));
    ...
    function actionAjaxSearchDocument()
    {
        ...
        foreach ($list as $document) {
            $json[] = array(
                'label' => t('Документ №%id от %date', array(
                    'id' => $document['id'],
    ...
    function actionOpenArchiveWindow()
    {
        ...
        if ($this->url->isPost()) {
            ...
            if($no_error === true){
                return $this->result
                    ->addSection('callCrudAdd', $this->router->getAdminUrl('ProcessArchive', array('date' => $date, 'mode' => $mode)))
                    ->setSuccess(true);
            }else{
        ...
        $helper = new CrudCollection($this, null, null, array(
            ...
            'bottomToolbar' => new Toolbar\Element(array(
                    'Items' => array(
                        new ToolbarButton\SaveForm($this->router->getAdminUrl('OpenArchiveWindow', array(), 'catalog-inventoryctrl'), t('Начать')),
                        new ToolbarButton\Cancel($this->router->getAdminUrl('edit', array('mod' => 'catalog'), 'modcontrol-control'), t('Закрыть'))
                    ))
            ),
            'viewAs' => 'form'
        ));
