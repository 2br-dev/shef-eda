...
class InventoryManager implements \Catalog\Model\StockInterface
{
    ...
    function recalculateStocks($item, $warehouse_id)
    {
        ...
        $x_stock = \Catalog\Model\Orm\Xstock::loadByWhere(array(
    ...
    function updateRemainsFromOrder(\Shop\Model\Orm\Order $order, $flag, $old_warehouse = null)
    {
        ...
        $applied = $cancelled ? 0 : $applied;
        $applied = $resume ? 1 : $applied;

        $finished_order = (in_array($order['status'], $success_status) && !in_array($order->this_before_write['status'], $success_status));

        $manager = new \Catalog\Model\DocumentLinkManager();
        $links = $manager->getLinks(array('type' => $order::DOCUMENT_TYPE_ORDER, 'id' => $order['id']));
    ...
    function getStartNums($product_id, $offer_id = null, $warehouse_id = null)
    {
        ...
        if($offer_id){
            $q->where(array('offer_id' => $offer_id,));
        }
        if($warehouse_id){
            $q->where(array('warehouse_id' => $warehouse_id,));
        }
