...
use \Catalog\Model\Orm\Property\Item,
    \Catalog\Model\Orm\Property\ItemValue,
    \Catalog\Model\Orm\Xdir,
...
class Api extends \RS\Module\AbstractModel\EntityList
{
    ...
    protected function isNumFilter($filter)
    {
        if ($filter != "") {
            ...
            if (ModuleManager::staticModuleExists('affiliate')
                && $config['affiliate_stock_restriction']
    ...
    */
    ...
    public static function decodeDirFilterParamsFromUrl()
    {
        ...
        if ($url_filtes_cache === null){
            ...
            if ($url_filters_part && !$config['use_seo_filters']) {
                throw new Exception(t('ЧПУ фильтры выключены.'));
            }
                ...
                if (!empty($url_filters)){
                    ...
                    foreach ($url_filters as $property_dataparts){
                        ...
                        if ($property_id){ //id характеристики
                            ...
                            throw new Exception(t('Таких характеристик не существует.'));
    ...
    private static function getEncodeDirFilterParamsArrayUrlParts($bfilters = array(), $pf = array(), $query = "")
    {
        ...
        $config = \RS\Config\Loader::byModule('catalog'); //Конфиг модуля
    ...
    public function setAffiliateRestrictions($set_filter_by_warehouse = false)
    {
        ...
        if (ModuleManager::staticModuleExists('affiliate')
            ...
            $warehouse_ids = $current_affiliate->getLinkedWarehouses();
