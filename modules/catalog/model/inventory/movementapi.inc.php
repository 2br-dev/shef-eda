...
class MovementApi extends \RS\Module\AbstractModel\EntityList
{
    ...
    function checkSaveErrors($document, $products)
    {
        if($document['warehouse_from'] == $document['warehouse_to']){
            return array('field' => 'warehouse_to', 'text' => t('Начальный и конечный склады не могут совпадать'));
        }
        if(!count($products)){
            return array('field' => 'products', 'text' => t('Не добавлено ни одного товара'));
        }
        ...
        foreach ($products as $uniq => $product){
            ...
            if($num - $product['amount'] < 0){
                ...
                return array('field' => 'products', 'text' => t('Количество товара "%0" после перемещения станет отрицательным', array($product_orm['title'])));
            }
            if($product['offer_id'] == -1){
                return array('field' => 'products', 'text' => t('Не у всех товаров выбрана комплектация'));
            }
        }
        return true;
    }
    ...
    {
        return new \Catalog\Model\Orm\Product($product_id);
    }
