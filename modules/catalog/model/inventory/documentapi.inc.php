...
class DocumentApi extends EntityList
{
    ...
    function getDocumentPrintForm($document_id, $document_type)
    {
        ...
        $provider = $document['provider'] ? new \Users\Model\Orm\User($document['provider']) : false;
        if($provider){
            if($provider['company']){
     ...
     * @return Document
     */
    function saveDocumentFromOrder(\Shop\Model\Orm\Order $order, $items, $applied, $document_type)
    {
        $warehouse = $order['warehouse'] ? $order['warehouse'] : \Catalog\Model\WareHouseApi::getDefaultWareHouse()->id;
        $link_manager = new \Catalog\Model\DocumentLinkManager($order['id'], $order::DOCUMENT_TYPE_ORDER);
        $links = $link_manager->getLinks();
        if($links){
            $document = $this->updateDocument($items , $warehouse, $order['dateof'], $links[0]['document_id'], $applied, $document_type);
        }else{
            $document = $this->createDocument($items, $document_type, $warehouse, $order['dateof'], $applied);
        }
        return $document;
    }
     ...
     * @return array
     */
    function prepareItemsFromOrder(\Shop\Model\Orm\Order $order)
    {
        $new_items = array();
        $cart = $order->getCart();
        ...
        foreach ($items as $uniq => $item){
            ...
            $offer_id = $item['cartitem']['offer'] !== null ? $offers['items'][$item['cartitem']['offer']]['id'] : \RS\Orm\Request::make()
                ->select()
                ->from(new \Catalog\Model\Orm\Xstock())
                ->where(array(
                    'product_id' => $item['cartitem']['product_id'],
                    'warehouse_id' => $order['warehouse'] ? $order['warehouse'] : \Catalog\Model\WareHouseApi::getDefaultWareHouse()->id,
                ))
                ->exec()
