...
class InventoryTools extends BaseModel
{
    ...
    static function staticSelectProviders()
    {
        ...
        return array_merge(array(0 => t('Не выбрано')), $result);
    }
    ...
    function prepareItemsFromCsv($row, $type = null)
    {
        ...
        if(count($array) > 1){
            ...
            $product = Product::loadByWhere(array($field_identificate => $product_identificator));

            if(!$offer_identificator){
                $offers = $product->fillOffers();
                $offer_id = $offers['items'][0]['id'];
            }else{
                $offer = Offer::loadByWhere(array($field_identificate => $offer_identificator));
                $offer_id = $offer['id'];
            }
    ...
    function setToZeroStocks($product_id)
    {
        \RS\Orm\Request::make()
            ...
            ->set(array('num' => 0, 'reserve' => 0, 'waiting' => 0, 'remains' => 0))
            ->where(array('product_id' => $product_id))
            ->exec();
        \RS\Orm\Request::make()
            ->update(new \Catalog\Model\Orm\Xstock())
            ->set(array('stock' => 0, 'reserve' => 0, 'waiting' => 0, 'remains' => 0))
            ->where(array('product_id' => $product_id))
            ->exec();
    }
    ...
    function checkArchiveErrors($date, $do)
    {
        ...
        if(!$amount){
            return array('error' => t('Нет документов на указанную дату'));
        }
        return true;
    }
    ...
    function getDocsAmountToArchive($params, $date)
    {
        ...
        $doc_amount = $q->whereIn('type', array(
    ...
    function markArchivedDocuments($params, $docs_id_arr)
    {
        \RS\Orm\Request::make()
            ->update()
            ->from(new Document())
            ->set(array('archived' => $params['archived'] == 1 ? 0 : 1))
