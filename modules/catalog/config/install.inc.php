    ...
    function deferredAfterInstall($options)
    {
        if ($options['set_demo_data']) {
            ...
            if ($site_config->getThemeName() == 'default') {
                ...
                PageApi::setupModule('main.index', 'catalog\controller\block\topproducts', array(
                    'dirs' => $top_dir_id
                ));
                
                //Настраиваем блок товары в виде баннера
                PageApi::setupModule('main.index', 'catalog\controller\block\bannerview', array(
                    'categories' => array(
                ...
                $actual_dir = Dir::loadByWhere(array(
                    'alias' => 'actual'
                ));
                if ($actual_dir['id']) {
