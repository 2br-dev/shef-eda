...
abstract class AbstractOfferType
{
    ...
    * @param string $key - title характеристики
    */
            ...
            */
            if (isset ($product['multioffers']['levels'])) {
                foreach ($product['multioffers']['levels'] as $item) {
                    if ($item['title'] == $key) {
                        $prop = new \Catalog\Model\Orm\Property\Item($item['prop_id']);
                        $cachename[$key] = $prop->name_for_export ?: $prop['id'];
                    }
                }
            }
            else {
                ...
                $cachename[$key] = ($prop->name_for_export) ?: $key;
            }
        }
        ...
        foreach($product['offers']['items'] as $item){
            if ($item['sortn'] == $offer_index){
                ...
                foreach ($item['photos_arr'] as $imageid ) {
                    ...
                    if  ($n<10){
                        $image_url = ($profile['export_photo_originals']) ? $image->getOriginalUrl() : $image->getUrl(800, 800, 'axy');
                        $writer->writeElement('picture', \RS\Http\Request::commonInstance()->getDomain(true) . $image_url);
                        $n++;
                    }
                }
                break;
