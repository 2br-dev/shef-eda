    ...
    */
    function getCost($use_currency = false, $format = false)
    {
        ...
        if($use_currency){
            ...
            return $format ? \RS\Helper\CustomView::cost($cost, $base_currency['stitle']) : $cost;
        }
    }
    ...
    function onResult(\RS\Http\Request $request)
    {
        ...
        try{
            ...
            if (!$is_result_exception || $e->canUpdateTransaction()) {
                ...
                //Только при первой попытке оплаты обновляем транзакцию
                if($start_status == self::STATUS_NEW) {
                    $this->update();
                }
            }
        ...
        //Выполняем действие только при первой попытке оплаты
        if($start_status == self::STATUS_NEW) {
