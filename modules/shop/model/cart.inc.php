...
class Cart
{
    ...
    protected function addTaxData($result)
    {
        ...
        if ($this->mode != self::MODE_SESSION && $this->order ) {
            ...
            foreach($this->getProductItems() as $key => $product) {
                ...
                $event_result = \RS\Event\Manager::fire('cart.getcartdata.producttaxlist', array(
                    'taxes' => $taxes,
                    'product' => $product,
                    'cart' => $this,
                ));
    ...
    protected function addProductData($result, $use_currency)
    {
        ...
        foreach($in_basket as $n => $item) {
            ...
            $result['items'][$n] = array(
                ...
                'base_cost' => $complete_cost, //Данная цена будет использоваться в качестве исходной для расчета скидки. изменяться не будет
                'amount' => (float)$amount, // Количестово товара
            ...
            foreach($concomitants as $sub_product) {
                ...
                (
                    ...
                    'cost' => $sub_product_cost,//Конечная цена для отображения, данная цена будет изменяться скидками далее
                    'base_cost' => $sub_product_cost,//Данная цена будет использоваться в качестве исходной для расчета скидки. изменяться не будет
    ...
    protected function addDiscountData($result, $use_currency)
    {
        ...
        foreach($coupon_items as $key => $item) {
            ...
            if ($use_currency){
                $min_order_price = CurrencyApi::applyCurrency($min_order_price);
            }
            if ($min_order_price > $result['total']) {
                ...
                if (empty($coupon_usage[$key]['products']) && !$item['cartitem']->getExtraParam('ignore_usage')) {
                    ...
                    switch ($coupon['discount_type']) {
                        case Discount::DISCOUNT_TYPE_PERCENT:
                            ...
                            break;
                        case Discount::DISCOUNT_TYPE_BASE_CURRENCY:
                            $max = $shop_config['fixed_discount_max_order_percent'];
                            $max_item = $shop_config['fixed_discount_max_order_item_percent'];
                            $result = $this->addFixedDiscountData($result, $coupon['discount'], $max, $max_item, $coupon_usage[$key]);
                    }
                }
            }
        }
    ...
    private function getProductPrice($product, $offer, $amount, $cost_id, $use_currency)
    {
        ...
        if ($use_currency) $cost = CurrencyApi::applyCurrency($cost);
...
//            }
    ...
    protected function addInternalProductDiscount($result, $use_currency)
    {
        ...
        foreach($in_basket as $n => $item) {
            ...
            if (isset($extra['personal_discount'])) {
                ...
                $discount = CostApi::roundCost($discount, 'round');
            ...
            if (isset($extra['sub_products_discount'])) {
                foreach ($extra['sub_products_discount'] as $subid=>$sub_product_discount) {
                    ...
                    $discount = CostApi::roundCost($discount, 'round');
                    if (empty($result['items'][$n]['sub_products'][$subid]['discount']) || $discount > $result['items'][$n]['sub_products'][$subid]['discount']) {

                        if (empty($result['items'][$n]['sub_products'][$subid]['discount'])) {
                            $delta = $discount;
                        } else {
    ...
    function getCouponUsedTimesByCurrentUser(Discount $coupon){
        ...
        $cnt = \RS\DB\Adapter::sqlExec('SELECT COUNT(*)as cnt FROM('.OrmRequest::make()
                ->select('I.*,O.user_id')
    ...
    function repeatCartFromOrder($order_num)
    {
        ...
        if ($order){ //Если заказ нашли, то получим его корзину и попрбуем добавить теже товары в текущую корзину
            ...
            if (Auth::isAuthorize() && (Auth::getCurrentUser()->id != $order['user_id'])){
