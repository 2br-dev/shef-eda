...
abstract class AbstractTorgForm extends AbstractPrintForm
{
    ...
    function getHtml()
    {
        ...
        foreach($cart->getProductItems() as $key => $product) {
            $product_subtotal = $product['cartitem']['single_cost']; //стоимость товара без налогов
            ...
            foreach($taxes as $tax) {
                ...
                if(!$nds_found && $tax->is_nds){
                    ...
                    $result['items'][$key]['taxes']['value'] = $tax_value * $product['cartitem']['amount'];
                    $result['items'][$key]['subtotal'] = (float)(($product_subtotal) * $product['cartitem']['amount']);
                    $result['items'][$key]['taxes']['cost'] = ($product_subtotal + $tax_value) * $product['cartitem']['amount'];
