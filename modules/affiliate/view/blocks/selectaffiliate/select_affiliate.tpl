...
{if $current_affiliate.id}
    ...
    <span class="citySelect">
        ...
        <a data-href="{$router->getUrl('affiliate-front-affiliates', ['referer' => $referer])}" class="cityLink inDialog" data-need-recheck="{$need_recheck}" {$current_affiliate->getDebugAttributes()}>{$current_affiliate.title}</a>
    </span>
{/if}