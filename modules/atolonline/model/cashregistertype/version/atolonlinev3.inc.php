...
class AtolOnlineV3 extends \Shop\Model\CashRegisterType\AbstractType
{
    ...
    */
    protected function getReceiptsFromOrder()
    {
        ...
        // Событие для изменения данных чека
        $event_result = \RS\Event\Manager::fire('atolonline.receiptsfromorder.before', array(
            'list' => $list,
            'cartdata' => $cartdata,
        ));
        list($list, $cartdata) = $event_result->extract();
        ...
        for($i=0; $i<$count; $i++){
            ...
            foreach ($products as $n=>$item){
                ...
                */
                ...
                //ФФД 1.05-1.1
                $payment_object = $product['payment_subject'];
