...
<div style="white-space:nowrap" id="theme_block">
    <input name="{$elem.__theme->getFormName()}" value="{$elem.__theme->get()|escape}" {if $elem.__theme->getMaxLength()>0}maxlength="{$elem.__theme->getMaxLength()}"{/if} {$elem.__theme->getAttr()} />&nbsp;<a id="selectTheme" class="btn btn-default va-middle">{t}выбрать{/t}</a>
