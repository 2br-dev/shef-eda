...
{block name="content"}
    ...
    {t alias="Партнерский сайт - Письмо оформления заказа. Фраза уважаемый партнер."}Уважаемый, партнер! На сайте {$url->getDomainStr()} оформлен заказ.{/t}<br>
    {t}Номер заказа{/t}: <strong>{$order.order_num}</strong> от <strong>{$order.dateof|dateformat:"@date @time:@sec"}</strong>
