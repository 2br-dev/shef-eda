...
class CustomFields extends FilterAbstractType
{
    ...
    function __construct($key, $user_fields_manager, $object_type_alias, array $options = array())
    {
    ...
    protected function where_ids()
    {
        ...
        $q = Request::make()
                ->select('object_id')
                ->from(new CustomData())
                ->where(array('object_type_alias' => $this->getObjectTypeAlias()));
        ...
        foreach($values as $key => $value) {
            ...
            if (isset($structure[$key])) {
                ...
                if ($structure[$key]['type'] == UserFieldsManager::TYPE_STRING
                    ...
                    $query->where("(`value_string` like '%#string%' AND `field`='#key')", array(
                        'string' => $value,
                        'key' => $key
                    ));
                }
                ...
                elseif ($structure[$key]['type'] == UserFieldsManager::TYPE_LIST) {
                    $query->where("(`value_string` = '#string' AND `field`='#key')", array(
                        'string' => $value,
                        'key' => $key
                    ));
                }
                ...
                elseif ($structure[$key]['type'] == UserFieldsManager::TYPE_BOOL) {
                    $query->where("(`value_float` = '#float' AND `field`='#key')", array(
                        'float' => (integer)$value,
                        'key' => $key
                    ));
                }
        ...
        if (!$ids) {
            $ids = array(0); //Если ничего не найдено добавим невыполнимое условие
        }
