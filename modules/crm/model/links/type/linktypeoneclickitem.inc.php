...
class LinkTypeOneClickItem extends AbstractType
{
    ...
    {
        $form = new FormObject(new PropertyIterator(array(
            'one_click_id' => new SelectOneClickItem(array(
                'description' => t('Поиск по покупкам в 1 клик'),
                'crossMultisite' => true,
                'checker' => array('ChkEmpty', t('Покупка в 1 клик не выбрана')),
                'attr' => array(array(
    ...
    public function getLinkText()
    {
        if ($this->linked_object['id']) {
            return t('Покупка в 1 клик N%num от %date', array(
                'num' => $this->linked_object['id'],
                'date' => date('d.m.Y', strtotime($this->linked_object['dateof']))
            ));
        } else {
            return t('Покупка в 1 клик не найдена (ID: %id)', array(
    ...
    {
        if ($this->linked_object['id']) {
            $url = Manager::obj()->getAdminUrl('edit', array('id' => $this->linked_object->id), 'catalog-oneclickctrl');
            return $url;
        }
    }
}