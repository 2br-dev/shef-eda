...
class LinkTypeUser extends AbstractType
{
    ...
    public function getTabForm()
    {
        $form = new FormObject(new PropertyIterator(array(
            'user_id' => new TypeUser(array(
                'description' => t('Поиск по пользователю'),
                'checker' => array('ChkEmpty', t('Пользователь не выбран')),
                'attr' => array(array(
    ...
    public function getLinkText()
    {
        if ($this->linked_object['id']) {
            return t('Пользователь %user_fio', array(
                'user_fio' => $this->linked_object->getFio(),
            ));
        } else {
            return t('Пользователь не найден (ID: %id)', array(
    ...
    {
        if ($this->linked_object['id']) {
            $url = Manager::obj()->getAdminUrl('edit', array('id' => $this->linked_object->id), 'users-ctrl');
            return $url;
        }
    }
}