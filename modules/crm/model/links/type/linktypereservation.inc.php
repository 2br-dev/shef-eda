...
class LinkTypeReservation extends AbstractType
{
    ...
    public function getTabForm()
    {
        $form = new FormObject(new PropertyIterator(array(
            'reservation_id' => new SelectReservation(array(
                'description' => t('Поиск по предзаказам'),
                'crossMultisite' => true,
                'checker' => array('ChkEmpty', t('Предзаказ не выбран')),
                'attr' => array(array(
    ...
    public function getLinkText()
    {
        if ($this->linked_object['id']) {
            return t('Предзаказ N%num от %date', array(
                'num' => $this->linked_object['id'],
                'date' => date('d.m.Y', strtotime($this->linked_object['dateof']))
            ));
        } else {
            return t('Предзаказ не найден (ID: %id)', array(
    ...
    {
        if ($this->linked_object['id']) {
            $url = Manager::obj()->getAdminUrl('edit', array('id' => $this->linked_object->id), 'shop-reservationctrl');
            return $url;
        }
    }
