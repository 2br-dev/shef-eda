...
class LinkTypeOrder extends AbstractType
{
    ...
    public function getTabForm()
    {
        $form = new FormObject(new PropertyIterator(array(
            'order_id' => new SelectOrder(array(
                'description' => t('Поиск по заказу'),
                'crossMultisite' => true,
                'checker' => array('ChkEmpty', t('Заказ не выбран')),
                'attr' => array(array(
    ...
    public function getLinkUrl()
    {
        if ($this->linked_object['id'] > 0) {
            $url = Manager::obj()->getAdminUrl('edit', array('id' => $this->linked_object->id), 'shop-orderctrl');
            return $url;
        }
    }
