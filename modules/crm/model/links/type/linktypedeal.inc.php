...
class LinkTypeDeal extends AbstractType
{
    ...
    public function getTabForm()
    {
        $form = new FormObject(new PropertyIterator(array(
            'deal_id' => new SelectDeal(array(
                'description' => t('Поиск по сделкам'),
                'checker' => array('ChkEmpty', t('Сделка не выбрана'))
            ))
        )));
    ...
    public function getLinkText()
    {
        ...
        if ($this->linked_object['id']) {
            return t('Сделка N%num от %date', array(
                'num' => $this->linked_object['deal_num'],
                'date' => $this->linked_object['date_of_create']
            ));
        } else {
            return t('Сделка не найдена (ID: %id)', array(
    ...
    public function getLinkUrl()
    {
        if ($this->linked_object['id'] > 0) {
            $url = Manager::obj()->getAdminUrl('edit', array('id' => $this->linked_object->id), 'crm-dealctrl');
            return $url;
        }
    }
}