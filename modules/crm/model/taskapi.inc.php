...
class TaskApi extends AbstractLinkedApi
{
    ...
    {
        parent::__construct(new Orm\Task(), array(
    ...
    public function getFilterControl()
    {
        ...
        return new Filter\Control( array(
            'Container' => new Filter\Container( array(
                'Lines' =>  array(
                    new Filter\Line( array('items' => array(
                        new Filter\Type\Text('task_num', t('Номер'), array('SearchType' => '%like%')),
                        new Filter\Type\Text('title', t('Короткое описание'), array('SearchType' => '%like%')),
                        new Filter\Type\User('creator_user_id', t('Создатель')),
                        new Filter\Type\User('implementer_user_id', t('Исполнитель')),
                        new Filter\Type\Select('status_id', t('Статус'), StatusApi::staticSelectList(array('' => t('Любой')), 'crm-task')),
    ...
    public function applyFilter(TaskFilter $filter)
    {
        ...
        $filter_control->fill($filter['filters_arr'] ?: array());
