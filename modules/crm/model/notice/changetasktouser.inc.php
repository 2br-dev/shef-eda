    ...
    implements InterfaceEmail, InterfaceDesktopApp, InterfaceSms
{
    ...
    public function getNoticeDataDesktopApp()
    {
        if (isset($this->recipients_data['user_ids'][0])) {
            ...
            $notice_data->title = t('Изменена задача №%0', array($this->task->task_num));

            $notice_data->destination_user_id = $this->recipients_data['user_ids'][0];

            $notice_data->short_message = t('%title %creator', array(
                'title' => $this->task['title'],
                'creator' => ($this->task['creator_user_id'] ? t('от %0', array($this->task->getCreatorUser()->getFio())) : '')
            ));
            $notice_data->link = \RS\Router\Manager::obj()->getAdminUrl('edit', array('id' => $this->task['id']), 'crm-taskctrl', true);
    ...
    public function getNoticeDataEmail()
    {
        ...
        $notice_data->subject   = t('Изменена задача №%0', array($this->task['task_num']));
        $notice_data->vars      = $this;
