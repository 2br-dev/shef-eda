    ...
    implements InterfaceEmail, InterfaceDesktopApp, InterfaceSms
{
    ...
    public function getNoticeDataDesktopApp()
    {
        ...
        $notice_data->title = t('Срок выполнения задачи №%num (%title) скоро подходит к концу', array(
        ...
        $notice_data->short_message = t('Осталось %time', array(
            'title' => $this->task['title'],
    ...
    public function getNoticeDataEmail()
    {
        ...
        $notice_data->subject   = t('Срок выполнения задачи №%num (%title) скоро подходит к концу', array(
            'num' => $this->task->task_num,
            'title' => $this->task->title
        ));
        $notice_data->vars      = $this;
