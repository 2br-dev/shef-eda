...
class TaskBoardItem extends AbstractBoardItem
{
    ...
    public function getFilters()
    {
        ...
        $filters = $task_filters_api->getSelectList(array(0 => t('Все')));
     ...
     * @return array
     */
    public function getButtons($filters = array())
    {
        $params = array();
        if (!empty($filters['preset_id'])) {
            $taskFilter = new TaskFilter($filters['preset_id']);
            $params = array('dir' => $taskFilter['id'], 'f' => $taskFilter['filters_arr']);

        }

        $router = RouterManager::obj();
        return array(
            new ToolbarButton\Add($router->getAdminUrl('add', array(), 'crm-taskctrl'), t('Добавить задачу')),
     ...
     * @return EntityList
     */
    public function getApiWithFilters($status_id, $filters = array())
    {
