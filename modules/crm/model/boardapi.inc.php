...
class BoardApi
{
    ...
    public static function getAllBoardItems()
    {
        ...
        if (!isset($board_item_types)) {
            $board_item_types_list = Manager::fire('crm.getboardtypes', array())->getResult();

            $board_item_types = array();
    ...
    public static function makeBoardItemByObjectType($object_type)
    {
        ...
        if (!isset($board_item_types[$object_type])) {
            throw new \RS\Exception(t('Тип элементов с идентификатором `%0` не зарегистрирован', array($object_type)));
        }
