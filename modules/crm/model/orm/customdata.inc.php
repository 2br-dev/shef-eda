...
class CustomData extends AbstractObject
{
    ...
    function _init()
    {
        $this->getPropertyIterator()->append(array(
            ...
            'value' => new Type\Text(array(
                'description' => t('Текстовое значение')
            ))
        ));

        $this->addIndex(array('object_type_alias', 'object_id', 'field'), self::INDEX_PRIMARY);

        $this->addIndex(array('object_type_alias', 'object_id', 'value_float'), self::INDEX_KEY);
        $this->addIndex(array('object_type_alias', 'object_id', 'value_string'), self::INDEX_KEY);
    }
