...
abstract class AbstractSchema extends BaseModel
{
    ...
    function __construct($base_preset, array $other_presets = array(), $options = array())
    {
        ...
        EventManager::fire('csv.scheme.afterconstruct.'.$this->getShortName(), array('scheme' => $this));
    }
        ...
        while($offset == 0 || count($this->rows)) {
            ...
            foreach($this->rows as $n => $row) {
                ...
                foreach($this->presets as $preset) {
                    ...
                    EventManager::fire('csv.beforelineexport.'.$class_short_name,array(
                        'schema' => $this
                    ));
    ...
    */
    function import($file, $check_timeout = false, $start_pos=0, $site_id = null)
    {
        ...
        $class_short_name = $this->getShortName();
        if ($start_pos==0){
           EventManager::fire('csv.beforeimport.'.$class_short_name,array(
                'schema' => $this
           )); 
        }
        ...
        while($fp                                  
            ...
            EventManager::fire('csv.beforelineimport.'.$class_short_name, array(
                'schema' => $this
            ));
            ...
            EventManager::fire('csv.afterlineimport.'.$class_short_name, array(
                'schema' => $this
            ));
        ...
        EventManager::fire('csv.afterimport.'.$class_short_name, array(
            'schema' => $this
        ));
    ...
    function getUploader()
    {
        if (!isset($this->uploader)) {
            ...
            $this->uploader->setRightChecker(function($uploader, $post_file_arr) use ($self) {
                ...
                return Rights::CheckRightError($self, DefaultModuleRights::RIGHT_CREATE);
            });
        }
