...
abstract class TreeList extends EntityList
{
    ...
    * @return array
    */
    public function getSelectList($parent_id = 0, array $first = array(), $level = 0)
    {
        $tmp = array();
        if (!isset($this->cat)) $this->getAllCategory();
        if (!empty($this -> catByParent[$parent_id])) {
            foreach ($this -> catByParent[$parent_id] as $v) {
                $tmp[$v[$this -> id_field]] = str_repeat('&nbsp;', $level*4).$v[$this -> name_field];
                $tmp = $tmp + $this -> getSelectList($v[$this -> id_field], $first,  $level + 1);
            }
        }
        if (!empty($first) && $level == 0) {
            $tmp = $first + $tmp;
        }
