...
abstract class AbstractModuleRights
{
    ...
    final protected function __construct($module)
    {
        ...
        $additional_rights = \RS\Event\Manager::fire($event_name, array())->getResult();
    ...
    */
    final protected function addRights($rights, $parents = array())
    {
        $parent = end($parents);
        ...
        foreach ($rights as $item) {
            ...
            if ($item instanceof RightGroup) {
                ...
                $parents[] = $item;
                $this->addRights($item->getChilds(), $parents);
            }
        }
    }
